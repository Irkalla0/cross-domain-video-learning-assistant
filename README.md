...
Turn Douyin / Bilibili / YouTube / TikTok videos into actionable learning plans.
...
```
4. 将生成的 `transcript_artifacts/run-*/transcript.txt` 作为 `subtitle_text` 输入 skill。
...
### Features
- Input: `video_url` + subtitles (recommended) + top comments (optional) + user goal (optional)
- Output: structured report (evidence index, action checklist, domain module, risk boundaries, reflection prompts)
...
### Dictation / Transcription Languages
Two layers:
1. `yt-dlp` subtitle layer: any language exposed by source subtitle tracks
2. `whisper` fallback layer: multilingual STT (common: `zh`, `en`, `ja`, `ko`, `es`, `fr`, `de`, `ru`, `ar`, `hi`)
...
If no subtitle track is available:
```bash
...
Use `transcript_artifacts/run-*/transcript.txt` as `subtitle_text` for skill analysis.
